...
public class ProtobufToMultiLabelDataset {
  ...
  static class Mention {
	...
	public static MultiLabelDataset<String, String> toMultiLabelDataset(InputStream is) throws IOException {
	  List<RelationAndMentions> relations = toRelations(is, true);  // 读入protoBuf, 整理成relations
	  MultiLabelDataset<String, String> dataset = toDataset(relations);  // 将整理的relations再塞入给定的datasets格式中.
	  return dataset;
	}
	
	public static void toDatums(InputStream is,  //这个函数测试时用
    ...
    while ((r = Relation.parseDelimitedFrom(is)) != null) {
      RelationAndMentions relation = new RelationAndMentions(     // 可见这里的relation表示ep. 注意relation.posLabels中不含NA. 如果只有NA的话, posLabels就是空集.
          r.getRelType(), r.getSourceGuid(), r.getDestGuid());    // 他应该用的是Hoffmann处理之后的一个ep对多个labels的数据集.(getRelType得到的是types)
      ...
      for(int i = 0; i < r.getMentionCount(); i ++) {
        Relation.RelationMentionRef mention = r.getMention(i);
    ...
    if(generateNegativeLabels) {
      for(RelationAndMentions rel: relations) {
        ...
        rel.negLabels = negatives;    // 所以, negLabels也不含NA. 更说明: NA relation的negLabels是全部labels.
      }
    }
    ...
    int [][][] data = new int[relations.size()][][];  // 构成形式: [ep_idx][mention_idx][feature_idx] = feature编号
    ...
    for(RelationAndMentions rel: relations) {
      ...
      posLabels[offset] = pos;    // 每个元素表示一个ep, 每个ep由其labels的index集合构成.(不是二元喔~)
